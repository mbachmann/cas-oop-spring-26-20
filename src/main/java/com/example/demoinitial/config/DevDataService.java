...
public class DevDataService implements HasLogger {
    ...
    @Autowired @Qualifier("felixMuster") Person felixMuster;
    @Autowired @Qualifier("maxMustermann") Person maxMustermann;
    ...
    private void createUserData() {
        if (userRepository.findByEmail("felix.muster@example.com").isEmpty()) {
    ...
    private Employee createEmployeeFelixMuster() {
        ...
        address.setCity("Zürich"); address.setState("ZH");
        address.setStreet("Lagerstrasse 41"); address.setZip("8004");
        Employee emp = new Employee();
        emp.setAddress(address); emp.setName("Felix Muster"); emp.setSalary(80000);
        return employeeRepository.save(emp);
    }

    private void createEmployeeJohnDoe() {
        Address address = new Address();
        address.setCity("Basel"); address.setState("BS");
        address.setStreet("Peter-Merianstrasse 41"); address.setZip("4001");
        Phone phoneWork = new Phone();
        phoneWork.setPhonenumber("061 123 45 67"); phoneWork.setType("Work");
        Employee emp = new Employee();
        emp.setAddress(address); emp.getPhones().add(phoneWork);
        emp.setName("John Doe"); emp.setSalary(100000);
    ...
    private void createEmployeeMaxMustermann() {
        Address address = new Address();
        address.setCity("Aarau"); address.setState("AG");
        address.setStreet("Bahnhofstrasse 41"); address.setZip("5001");
        Phone phoneWork = new Phone();
        phoneWork.setPhonenumber("062 888 45 67"); phoneWork.setType("Work");
        Employee emp = new Employee();
        emp.setAddress(address); emp.getPhones().add(phoneWork);
        emp.setName("Max Mustermann"); emp.setSalary(90000);
        phoneWork.setEmployee(emp);
        employeeRepository.save(emp);
    }
    ...
    private void createPhone(Long empId) {
        Phone phoneWork = new Phone();
        phoneWork.setPhonenumber("031 999 99 99"); phoneWork.setType("Work");
        Phone phoneHome = new Phone();
        phoneHome.setPhonenumber("032 333 33 33"); phoneHome.setType("Home");
        employeeRepository.findByIdWithEagerRelationships(empId).ifPresent(emp -> {
            phoneWork.setEmployee(emp); emp.getPhones().add(phoneWork);
            phoneHome.setEmployee(emp); emp.getPhones().add(phoneHome);
            employeeRepository.save(emp);
        });
    }

    private void createDepartment() {
        Department hr = new Department(); hr.setName("HR"); departmentRepository.save(hr);
        Department dev = new Department(); dev.setName("Dev");
        Department departmentDev = departmentRepository.save(dev);
        employeeRepository.findAllWithEagerRelationships().forEach(emp -> {
            departmentDev.getEmployees().add(emp); emp.setDepartment(departmentDev); employeeRepository.save(emp);
        });
    ...
    private void createDesignProject() {
        DesignProject project = new DesignProject("Arcos"); project.setInnovationLevel(3);
        employeeRepository.findAllWithEagerRelationships().forEach(emp -> {
            project.getEmployees().add(emp); emp.getProjects().add(project);
        });
    ...
    private void assignMaxMustermannToDesignProject() {
        employeeRepository.findByNameWithEagerRelationships("Max Mustermann").ifPresent(employee ->
                projectRepository.findWithEagerProjectByName("Arcos").forEach(project -> {
                    project.getEmployees().add(employee); employee.getProjects().add(project);
                    projectRepository.save(project);
                }));
    }
    ...
    private void assignMaxMustermannAsChef() {
        employeeRepository.findByNameWithEagerRelationships("Max Mustermann").ifPresent(chef ->
                employeeRepository.findByNameWithEagerRelationships("Felix Muster").ifPresent(employee -> {
                    employee.setChef(chef); chef.getEmployees().add(employee); employeeRepository.save(chef);
                }));
    }
}
